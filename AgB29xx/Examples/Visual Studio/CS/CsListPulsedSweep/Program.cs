...
namespace AgB29xx_CS_ListPulsedSweep
{
    /// <summary>
    ...
    class Program
    {
        ...
        {
            Console.WriteLine("  CS_ListPulsedSweep");
            Console.WriteLine();
            AgB29xx driver = null;
            ...
            try
            {
                ...
                for (int i = 1; i <= iNumberOfChannels; i++)
                {
                    ...
                    double[] daVoltageList = new double[] { -5, +5, -4, +4, -3, +3, -2, +2, -1, +2, 0 };
                    driver.Transients.get_Item(driver.Transients.get_Name(i)).Voltage.Mode = Agilent.AgB29xx.Interop.AgB29xxTransientCurrentVoltageModeEnum.AgB29xxTransientCurrentVoltageModeList;
                    driver.Transients.get_Item(driver.Transients.get_Name(i)).Voltage.ConfigureList(ref daVoltageList);

                    // Output shape, pulse delay, and pulse width.

                    driver.Outputs.get_Item(driver.Outputs.get_Name(i)).Shape = Agilent.AgB29xx.Interop.AgB29xxOutputShapeEnum.AgB29xxOutputShapePulse;
                    driver.Outputs.get_Item(driver.Outputs.get_Name(i)).PulseDelay = PULSE_DELAY;
                    driver.Outputs.get_Item(driver.Outputs.get_Name(i)).PulseWidth = PULSE_WIDTH;
                    ...
                    driver.Outputs.get_Item(driver.Outputs.get_Name(i)).Voltage.Level = 0.0;

                    // Set fixed range current measurement

                    driver.Measurements.get_Item(driver.Measurements.get_Name(i)).Trigger.Delay = PULSE_DELAY + PULSE_WIDTH - APERTURE_TIME;

                    if (str == "B2901A" || str == "B2902A" || str == "B2911A" || str == "B2912A" || str == "B2901B" || str == "B2902B" || str == "B2911B" || str == "B2912B")
                        driver.Measurements.get_Item(driver.Measurements.get_Name(i)).Current.AutoRangeEnabled = false;//Supported Models for this property: B2901A|B, B2902A|B, B2911A|B, B2912A|B 

                    driver.Measurements.get_Item(driver.Measurements.get_Name(i)).Current.Aperture = APERTURE_TIME;

                    // Set trigger condition

                    //trigger.TriggerCount = 11;
                    driver.Transients.get_Item(driver.Transients.get_Name(i)).Trigger.Count = 11;
                    driver.Measurements.get_Item(driver.Measurements.get_Name(i)).Trigger.Count = 11;
